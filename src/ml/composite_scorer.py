"""Unified Multi-Modal Risk Engine fusing ML, Anomaly, Velocity, and Rules."""
...
class UnifiedRiskScorer:
    """Fuses Supervised ML, Unsupervised Anomaly Detection, Velocity Signals, and Heuristic Rules."""
    ...
    def evaluate(self, features: FeatureVector) -> UnifiedRiskDecision:
        """Evaluates an incoming FeatureVector across ML models and behavioural rules to compute unified risk."""
        ...
        # Transaction velocity (TPS): 0.1 TPS is normal, >= 0.5 TPS is anomalous
        velocity_score = round(min(1.0, float(features.transaction_velocity) / 0.5), 3)

        # 4. Composite Scoring Fusion Formula
        # Weights: XGBoost (0.50), Isolation Forest (0.25), Velocity (0.25)
        w_xgb = 0.50
        w_iso = 0.25
        w_vel = 0.25
        ...
        # 5. Assign Bounded Defensive Action
        ...
        # 6. Extract Top Risk Signals (Combining Rules & SHAP)
        top_signals: List[str] = []
        for r in triggered_rules:
            if r.score_impact > 0:
                top_signals.append(f"[Rule Triggered] {r.rule_name} (+{r.score_impact:.0f} pts)")

        for b_point in shap_data.get("summary_bullet_points", [])[:3]:
            top_signals.append(f"[ML Driver] {b_point}")

        if not top_signals:
            top_signals.append("Organic baseline pattern with clean device and normal request velocity.")

        # 7. Identify Primary Rule Triggered
        ...
        # 8. Build Grounded Explainable Text
    ...
    def _build_explanation(
        ...
        sections.append(
            f"Model Scores: XGBoost Fraud Probability = {round(fraud_prob * 100, 1)}%, "
            f"Isolation Forest Anomaly Score = {round(anomaly_score * 100, 1)}%."
        )

        # Note legitimate flash sale discount if present
        flash_rules = [r for r in triggered_rules if r.severity == "DISCOUNT"]
        if flash_rules:
            sections.append(
                f"Notice: Traffic recognized as a high-entropy legitimate flash sale "
                f"(IP entropy: {features.merchant_ip_entropy}, Device entropy: {features.merchant_device_entropy}). "
                f"False positive protection discount applied."
            )
